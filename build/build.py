...
def generate_readme_pdf(dist_dir):
    """Generate README.pdf from README.md with embedded images.

    Requires pandoc and weasyprint. Skips gracefully if not available.
    """
    ...
    try:
        ...
        if e.stderr:
            print(f"    {e.stderr.strip().splitlines()[0]}")
        return False
    finally:
...
def build(version="DEV"):
    ...
    try:
        ...
        # Generate README.pdf with embedded images (optional — needs pandoc + weasyprint)
        if generate_readme_pdf(dist_dir):
            print(f"  ✓ README.pdf")

        # Remove images/ from dist (only needed for PDF generation)
